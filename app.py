...
# --- ABA 3: DASHBOARD (Com Alerta 90%, Evolução e Comparativo) ---
with aba3:
...
# --- ABA 4: INVESTIMENTOS (NOVA) ---
with aba4:
    st.subheader("📈 Gestão de Investimentos (Aportes)")
    st.info("Registre aqui os seus aportes e destinações para a meta de 20% de investimentos.")
    
    with st.form("form_investimento", clear_on_submit=True):
        col_i1, col_i2 = st.columns(2)
        with col_i1:
            ativo_inv = st.text_input("Ativo / Aplicação (Ex: Tesouro Direto, Ações, FIIs)")
            valor_inv = st.number_input("Valor do Aporte (R$)", min_value=0.0, value=0.00, step=1.0, format="%.2f")
        with col_i2:
            data_inv = st.date_input("Data do Aporte", value=date.today())
            
        if st.form_submit_button("Registrar Aporte", use_container_width=True):
            c.execute("INSERT INTO transacoes (data, tipo, descricao, categoria, valor) VALUES (?,?,?,?,?)",
                      (data_inv.strftime("%Y-%m-%d"), "Despesa", f"Inv: {ativo_inv}", "📈 Investimentos / Poupança (20%)", valor_inv))
            conn.commit()
            st.success("Aporte registrado com sucesso e computado nos seus investimentos!")

    st.markdown("---")
    st.subheader("📋 Extrato de Investimentos Realizados")
    df_trans_inv = pd.read_sql("SELECT * FROM transacoes WHERE categoria LIKE '%Investimentos%'", conn)
    if not df_trans_inv.empty:
        st.dataframe(df_trans_inv[['data', 'descricao', 'valor']], use_container_width=True)
    else:
        st.info("Nenhum investimento registrado até o momento.")
